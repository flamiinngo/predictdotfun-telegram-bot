...
class TelegramPredictBot:
    ...
    def detect_whale_activity(self, orders: List[Dict]) -> List[Dict]:
        ...
        for order in orders:
            ...
            # DEBUG: Show taker data structure to find side field
            if taker_data and isinstance(taker_data, dict):
                print(f"  🔍 Taker data keys: {list(taker_data.keys())}")
            
            # Get the side - try multiple possible field names
            order_side = None
            if isinstance(taker_data, dict):
                order_side = taker_data.get('side')
                if order_side is None:
                    # Try at order level
                    order_side = order.get('side')
                if order_side is None:
                    # Try as isBuy field
                    is_buy = taker_data.get('isBuy')
                    if is_buy is not None:
                        order_side = 0 if is_buy else 1
            
            print(f"  📊 Order side value: {order_side} (0=BUY, 1=SELL)")
            
            order_action = "BUY 🟢" if order_side == 0 else "SELL 🔴" if order_side == 1 else "UNKNOWN ⚪"
