...
class TelegramPredictBot:
    ...
    def detect_whale_activity(self, orders: List[Dict]) -> List[Dict]:
        ...
        print(f"\n{'='*60}")
        print(f"🔍 WHALE DETECTION DEBUG - Processing {len(orders)} orders")
        print(f"Current alerted_transactions set size: {len(self.alerted_transactions)}")
        print(f"{'='*60}\n")
        ...
        for idx, order in enumerate(orders, 1):
            ...
            # 🔍 DEBUG: Print everything about this potential whale bet
            print(f"--- Whale Bet #{idx} ---")
            print(f"  Amount: ${amount:.2f}")
            print(f"  Wallet: {wallet[:20] if wallet else 'NONE'}...")
            print(f"  Market: #{market_id}")
            print(f"  Side: {side_name}")
            print(f"  TX Hash: {tx_hash[:20] if tx_hash else 'EMPTY/NONE!'}")
            print(f"  TX Hash Type: {type(tx_hash)}")
            print(f"  TX Hash Bool: {bool(tx_hash)}")
            print(f"  In alerted set? {tx_hash in self.alerted_transactions if tx_hash else 'N/A - empty hash'}")
            ...
            if tx_hash and tx_hash in self.alerted_transactions:
                ...
                print(f"  ⚠️ WARNING: NO TX HASH! Cannot deduplicate properly!")
                print()
            else:
                print(f"  🆕 NEW whale bet - will process")
                print()
            ...
            # Get whale stats (simplified - just show bet count)
            whale_stats = self.get_whale_stats(wallet) if wallet else {}
            
            # Get market volume (from API if needed)
            ...
            if tx_hash:
                print(f"  ➕ Adding tx_hash to alerted set: {tx_hash[:20]}...")
                self.alerted_transactions.add(tx_hash)
                print(f"  ✅ Set now has {len(self.alerted_transactions)} items")
            else:
                print(f"  ❌ Cannot add to set - no tx_hash!")
        
        print(f"\n{'='*60}")
        print(f"📊 SUMMARY: Created {len(alerts)} whale alerts")
        print(f"Alerted transactions set final size: {len(self.alerted_transactions)}")
        print(f"{'='*60}\n")
