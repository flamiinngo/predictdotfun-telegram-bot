    ...
    def detect_whale_activity(self, orders: List[Dict]) -> List[Dict]:
        ...
        for order in orders:
            ...
            if amount >= self.whale_threshold:
                ...
                # ADD TO SET IMMEDIATELY to prevent reprocessing
                if tx_hash:
                    self.alerted_transactions.add(tx_hash)
                    print(f"  ➕ Added {tx_hash[:10]}... to tracking (will filter next)")
                ...
                print(f"  Analyzing entry quality...")
                ...
                print(f"  ✅ Alert created and saved!")
        
        print(f"  Summary: Created {len(alerts)} alerts, {len(self.alerted_transactions)} total txs tracked")
        return alerts
        ...
        while True:
            try:
                ...
                if orders:
                    ...
                    print(f"  Preparing to send {len(all_alerts)} alerts to Telegram...")
                    
                    for alert in all_alerts:
                        try:
                            # FINAL SAFETY CHECK: Don't send if we already sent this exact alert
                            if alert.get('type') == 'WHALE_BET':
                                ...
                                if alert_id:
                                    ...
                                    if tx_hash in self.alerted_transactions and tx_hash not in getattr(self, '_just_added', set()):
                                        print(f"  ⏭️ Skipping Telegram send - already alerted on {tx_hash[:10]}...")
                                        continue
                                    
                                    # Track what we just added this cycle
                                    if not hasattr(self, '_just_added'):
                                        self._just_added = set()
                                    self._just_added.add(tx_hash)
                            
                            await self.send_telegram_alert(alert, app)
                            print(f"  ✉️ Sent {alert.get('type')} alert to Telegram")
                        except Exception as e:
                            print(f"Error sending alert: {e}")
                    
                    # Clear the just_added set for next cycle
                    if hasattr(self, '_just_added'):
                        self._just_added.clear()
