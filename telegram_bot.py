...
class TelegramPredictBot:
    ...
    def load_alerted_transactions(self):
        ...
        # DISABLED: This was causing all alerts to be blocked
        # We'll rely on in-memory tracking which resets on restart
        # This is actually better - allows re-alerting after restarts if needed
        print(f"📊 Starting fresh - alerted transactions set is empty")
        return
    ...
    def detect_whale_activity(self, orders: List[Dict]) -> List[Dict]:
        ...
        for order in orders:
            ...
            if amount >= self.whale_threshold:
                ...
                # If no tx_hash, create fallback ID
                if not tx_hash:
                ...
                print(f"  🐋 WHALE FOUND! ${amount:.2f} on market #{market_id} ({side_name})")
                ...
                # NOW mark as alerted (only if it passed all filters)
                if tx_hash:
                    self.alerted_transactions.add(tx_hash)
                    print(f"  ✅ Alert created! Added {tx_hash[:10]}... to tracking")
        
        print(f"  Summary: {len(alerts)} alerts created, {len(self.alerted_transactions)} txs tracked this session")
        return alerts
