    ...
    def detect_whale_activity(self, orders: List[Dict]) -> List[Dict]:
        ...
        for order in orders:
            ...
            if amount >= self.whale_threshold:
                ...
                # CHECK if we've already alerted on this transaction (in-memory check)
                if tx_hash and tx_hash in self.alerted_transactions:
                    print(f"  ⏭️ Already alerted on tx {tx_hash[:10]}..., skipping...")
                    continue
                ...
                if tx_hash:
                    ...
                    print(f"  ✅ Added tx {tx_hash[:10]}... to alerted set")
