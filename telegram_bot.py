...
class TelegramPredictBot:
    ...
    def detect_whale_activity(self, orders: List[Dict]) -> List[Dict]:
        ...
        for order in orders:
            ...
            # Get the quoteType (Bid = buying, Ask = selling)
            quote_type = taker_data.get('quoteType') if isinstance(taker_data, dict) else None
            
            # Convert to uppercase for comparison (API returns "Bid"/"Ask" not "BID"/"ASK")
            quote_type_upper = quote_type.upper() if quote_type else None
            
            # BID = buyer (taking ask/buying shares) = BUY
            # ASK = seller (taking bid/selling shares) = SELL
            if quote_type_upper == 'BID':
                order_action = "BUY 🟢"
            elif quote_type_upper == 'ASK':
                order_action = "SELL 🔴"
            else:
                order_action = f"UNKNOWN ({quote_type})"
            
            print(f"  📊 Quote type: {quote_type} → {order_action}")
            ...
            category_slug = market_data.get('categorySlug', '') if isinstance(market_data, dict) else ''
            ...
            # Build proper URL: https://predict.fun/market/{categorySlug}
            # Note: The full slug isn't in API, so we link to category page where user can find the market
            if category_slug:
                market_url = f"https://predict.fun/market/{category_slug}"
            else:
            ...
            # FILTER 0: Skip markets that already closed (negative days only)
            if days_to_close is not None and days_to_close < 0:
                print(f"  ⏭️ SKIPPED: Market already closed ({days_to_close} days ago)")
                continue
    ...
    def format_alert(self, alert: Dict) -> str:
        ...
        if alert['type'] == 'WHALE_BET':
            ...
            order_action = alert.get('order_action', '')
            
            msg = f"🐋 *BIG WHALE ALERT!*\n\n"
            msg += f"❓ *Market Question:*\n`{alert.get('market_title', 'Unknown')}`\n\n"
            msg += f"{order_action} *${alert.get('amount', 0):.2f}* on *{alert.get('side')}*\n"
            ...
            msg += f"\n💡 *Look for Market #{alert.get('market_id')}*"
            ...
            market_title = alert.get('market_title', 'Unknown Market')
            ...
            msg += f"  • Fast-closing ✓\n\n"
            msg += f"🎯 *Strong Signal!*\n\n"
            msg += f"💡 *Search for Market #{alert.get('market_id')} on predict.fun*"
