    ...
    def detect_whale_activity(self, orders: List[Dict]) -> List[Dict]:
        ...
        for order in orders:
            ...
            if amount >= self.whale_threshold:
                ...
                # CREATE UNIQUE ALERT ID to prevent duplicate alerts
                # Use transaction hash + wallet + amount as unique identifier
                tx_hash = order.get('transactionHash', '')
                alert_id = f"{tx_hash}_{wallet}_{amount_str}"
                
                # CHECK if we've already alerted on this specific whale bet
                try:
                    self.cursor.execute('''
                    ...
                    if self.cursor.fetchone():
                        print(f"  ⏭️ Already alerted on this whale bet, skipping...")
                        continue
                except:
                    pass
